  ...
  return (
    <Card className="group overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 bg-white">
      ...
      <CardContent className="p-4">
        ...
        {/* Name */}
        <Link href={`/product/${product.id}`}>
          <h3 className="text-lg font-bold text-slate-800 mb-2 line-clamp-2 hover:text-blue-600 transition-colors">
            {product.name}
          </h3>
        </Link>
        ...
        <div className="flex items-center justify-between mb-4">
          <Badge
            ...
            className={product.stock_available ? 'bg-green-600' : 'bg-slate-200 text-slate-700'}
          >
        ...
        {product.rating && (
          <div className="flex items-center gap-1 mb-4">
            {[...Array(5)].map((_, i) => (
              <svg
                key={i}
                className={`w-4 h-4 ${i < product.rating ? 'text-yellow-400 fill-yellow-400' : 'text-slate-300'}`}
                viewBox="0 0 24 24"
              >
